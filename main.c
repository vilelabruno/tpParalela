...
void divide_range(image *binary_image, double factor) {
        ...
        while ((sd = readdir(dir))) {
            ...
            do {
                ...
                for (p = 0; (p < (ws - 1)) && (l->size > 0); p++) {
                    subimage_link *bl = outlink(l);
                    subimage_size[p][0] = (bl->y_end - bl->y_init);
                    subimage_size[p][1] = (bl->x_end - bl->x_init);
                    divide_subimage(row_pointers, subimage[p], bl->y_init, bl->y_end, bl->x_init, bl->x_end);
